...
export class TransferStyleHttpService {
    ...
    public changeAge(request: TransferStyleRequestModel, younger = false): Promise<any> {
        return this.http.request('post', this.general.getUrl(`/change-age?make_younger=${younger}`, ServicesType.STYLE_TRANSFER), request)!.toPromise();
    }
}